...
Supplier.all.each do |supplier|
  biz_conn = BusinessConnection.find_by_code(supplier.id)
  if biz_conn.nil?
    puts "#{supplier.id}, #{supplier.name}, Not Found"
  else
    s = str_similarity(supplier.name, biz_conn.name, {hiragana_to_katakana: true, convert_half_to_full: true, upcase: true})
    if s < 0.2
      puts "#{format('%.2f', s)}, #{supplier.id}, #{supplier.name}, #{biz_conn.name}"
    end
  end
end
